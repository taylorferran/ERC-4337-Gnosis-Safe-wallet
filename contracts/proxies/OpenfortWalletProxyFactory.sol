...
contract OpenfortWalletProxyFactory {
    ...
    function deployProxy(address _singleton, bytes memory initializer, bytes32 salt) internal returns (OpenfortWalletProxy proxy) {
        require(isContract(_singleton), "Singleton contract not deployed");

        bytes memory deploymentData = abi.encodePacked(type(OpenfortWalletProxy).creationCode, uint256(uint160(_singleton)));
    ...
    function createProxyWithNonce(address _singleton, bytes memory initializer, uint256 saltNonce) public returns (OpenfortWalletProxy proxy) {
    ...
    function createChainSpecificProxyWithNonce(
        ...
        uint256 saltNonce
    ) public returns (OpenfortWalletProxy proxy) {
        // If the initializer changes the proxy address should change too. Hashing the initializer data is cheaper than just concatinating it
